...
pub fn solve(order: []const u32, bingos: []Bingo) u32 {
    var index: usize = 0;
    while (true) {
        for (bingos) |bingo| if (bingo.checkWin()) |result| return order[index - 1] * result;
        for (bingos) |*bingo| bingo.draw(order[index]);
        index += 1;
    }
}
