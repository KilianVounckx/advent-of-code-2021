...
const Cavern = struct {
    ...
    pub fn parse(allocator: Allocator, string: []const u8) !Self {
        ...
        while (it.next()) |line| {
            var row = try allocator.alloc(u32, line.len * 5);
        ...
        const size = risks.items.len;
        var j: usize = size;
        while (j < size * 5) : (j += 1) {
            const old_row = risks.items[j - size];

            var row = try allocator.alloc(u32, old_row.len);
            errdefer allocator.free(row);
    ...
    pub fn format(
        ...
        for (self.risks) |row, i| {
            if (i != 0) try writer.print("\n", .{});
            for (row) |n| try writer.print("{d}", .{n});
        }
    }
